...
if os.name == "nt":
    ...
    class HandResultWin(Structure):
        ...
        ('clockLastDetect', ctypes.c_float),
...
"""
def enablePalmTracker():
"""
def enablePalmTracker(mode):
    global lib,obj, os, camID
    if os.name == "nt":
       return None
   
# Linux
    lib.aerotap_enablePalmTracker.argtypes = [ctypes.c_void_p,ctypes.c_bool]
    lib.aerotap_enablePalmTracker(obj,mode)
    return None

"""
def setPalmDetectMode():
"""
def setPalmDetectMode(mode):
    global lib,obj, os, camID
    if os.name == "nt":
       lib.AERO_SwitchDevice.argtypes = [ctypes.wintypes.INT]
       lib.AERO_SwitchDevice(camID)

       lib.AERO_SetZoom.argtypes = [ctypes.wintypes.INT]
       lib.AERO_SetZoom( 100 )


       lib.AERO_SetHandMode.argtypes = [ctypes.wintypes.INT]
       lib.AERO_SetHandMode(mode)
       print( "PalmDetectMode",mode)
       return None
   
# Linux
    lib.aerotap_setPalmDetectMode.argtypes = [ctypes.c_void_p,ctypes.c_int]
    lib.aerotap_setPalmDetectMode(obj,mode)
    return None

"""
def setPDepthRange():
"""
def setPDepthRange(nMin,nMax):
    global lib,obj, os, camID
    if os.name == "nt":
       lib.AERO_SetZoom.argtypes = [ctypes.wintypes.INT,ctypes.wintypes.INT,ctypes.wintypes.INT]
       lib.AERO_SetDepthRange(camID, nMin,nMax)
...
#        lib.AERO_SwitchDevice(camID)
        ...
        lib.AERO_DoHandDetect.argtypes = [ctypes.wintypes.INT]
 #       print( "doPalmTracking in")
        lib.AERO_DoHandDetect(1)
 #       print( "doPalmTracking out")

        return None
   
# Linex
    lib.aerotap_doPalmTracking.argtypes = [ctypes.c_void_p]
    lib.aerotap_doPalmTracking(obj)
    return None

"""
def getPalmTrackResult():
"""
def getPalmTrackResult():
    global lib,obj, os, camID,cHandResult
    if os.name == "nt":
        ...
        hr = HANDRESULT.unpack(buffer[:sz])
        cHandResult.centerX = hr[10]
        cHandResult.centerY = hr[11]
        cHandResult.centerZ = hr[12]
        cHandResult.rect_x = hr[0]
        cHandResult.rect_y = hr[1]
        cHandResult.rect_width = hr[2]
        cHandResult.rect_height = hr[3]
        cHandResult.objectType = hr[20]
        cHandResult.nDetected = hr[21]
        if hr[9]:
            cHandResult.used = True
            cHandResult.clockLastDetect = time.perf_counter_ns()
            return True
        elif cHandResult.used:
            if time.perf_counter_ns()-cHandResult.clockLastDetect>200:
                cHandResult.used = False
...
# Linux
    ...
    buffer = np.empty(sz*2,dtype=np.uint8)
    lib.aerotap_getPalmTrackResult.argtypes = [ctypes.c_void_p,ctypes.POINTER(ctypes.c_uint8)]
    if lib.aerotap_getPalmTrackResult(obj,buffer.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8))):
        hr = HANDRESULT.unpack(buffer[:sz])
#        print(hr)
        cHandResult.rect_x = hr[0]
        cHandResult.rect_y = hr[1]
        cHandResult.rect_width = hr[2]
        cHandResult.rect_height = hr[3]
        cHandResult.centerX = hr[5]
        cHandResult.centerY = hr[6]
        cHandResult.centerZ = hr[7]
        cHandResult.objectType = hr[10]
        cHandResult.nDetected = hr[11]
#        print("Hand detect!", cHandResult.centerX,cHandResult.centerY,cHandResult.centerZ,"rect", cHandResult.rect_x,cHandResult.rect_y,cHandResult.rect_width,cHandResult.rect_height)
        if hr[4]:
            cHandResult.used = True
            cHandResult.clockLastDetect = time.perf_counter_ns()
#            print("Hand detect!", cHandResult.centerX,cHandResult.centerY,cHandResult.centerZ,"rect", cHandResult.rect_x,cHandResult.rect_y,cHandResult.rect_width,cHandResult.rect_height)
...
if __name__ == "__main__":
   ...
   if bEnablePalmTrack:
       print("Enabling HandTrack mode...")      
       enablePalmTracker(True) # HANDMODE_CLICK
       setPalmDetectMode(10)
       setPDepthRange(0,1500)
...
# Loop Start 
   ...
   for i in range(400):
      if IsNewFrame():
           ...
           # Palm Tracking
           if bEnablePalmTrack and bDepthRead:
               doPalmTracking()
           ...
           if( not (img is None) ):
               if bShowFeedbackColor:
               # show feedback
                   if cHandResult.used:
                       print("Hand detect!",cHandResult.objectType, cHandResult.centerX,cHandResult.centerY,cHandResult.centerZ,"rect", cHandResult.rect_x,cHandResult.rect_y,cHandResult.rect_width,cHandResult.rect_height)
                       img = img.astype(np.uint8).copy() 
                       cv2.rectangle(img,pt1=(cHandResult.rect_x,cHandResult.rect_y),pt2=(cHandResult.rect_width,cHandResult.rect_height), color=(0, 255, 0),  thickness=3,  lineType=cv2.LINE_4,shift=0)
                   cv2.imshow( "aeroTAP camera View", img )
           if( not (depth is None) ):
           # convert depth map to bgr ColorImage
               bDepthRead = True;
               # show feedback
               if bShowFeedbackDepth:
                   DepthToRGB(depth)
